...
const canonicalServiceEntries = [
  ...
  [
    'opportunity-service',
    'opportunity',
    'Core Opportunity Service Contract Skeleton',
    'opportunity-service.md',
    'CORE-TASK-023'
  ],
  [
    'notification-service',
    'notification',
    'Core Notification Service Contract Skeleton',
  ...
  [
    'partner-service',
    'partner',
    'Core Partner Service Contract Skeleton',
    'partner-service.md',
    'CORE-TASK-023'
  ],
  [
    'agent-service',
    'agent',
    'Core Agent Service Contract Skeleton',
    'agent-service.md',
    'CORE-TASK-023'
  ],
  [
    ...
    'service-provider',
    'Core Service Provider Service Contract Skeleton',
    'service-provider-service.md',
    'CORE-TASK-023'
  ],
  [
    'service-network-service',
    'service-network',
    'Core Service Network Service Contract Skeleton',
  ...
  [
    'routing-service',
    'routing',
    'Core Routing Service Contract Skeleton',
    'routing-service.md',
    'CORE-TASK-023'
  ]
] as const;
const canonicalServiceSourceRoot =
...
export function validateCoreServiceContractSkeletons(
  contracts: readonly CoreServiceContract[]
): readonly string[] {
  if (!Array.isArray(contracts))
    return ['Core service contract skeletons must be an array.'];
  ...
  if (contracts.length !== 26)
    errors.push(
      'Core service contract skeletons must contain exactly 26 entries.'
    );
  ...
  contracts.forEach((contract, index) => {
    ...
    if (!Array.isArray(contract.nonGoals))
      errors.push(`${path}.nonGoals must be an array.`);
    ...
    if (typeof contract.serviceType === 'string') {
      if (!kebabCasePattern.test(contract.serviceType))
        errors.push(`${path}.serviceType must be kebab-case.`);
      if (serviceTypes.has(contract.serviceType))
        errors.push(`${path}.serviceType must be unique.`);
      serviceTypes.add(contract.serviceType);
    }
    if (
      typeof contract.domainId === 'string' &&
      !domainIds.has(contract.domainId)
    )
      errors.push(`${path}.domainId must exist in CORE_DOMAIN_REGISTRY.`);
    if (typeof contract.status === 'string' && !statuses.has(contract.status))
    ...
    if (promoted !== undefined) {
      if (contract.id !== promoted.id)
        errors.push(
          `${path}.id must match the promoted ${promoted.task} contract.`
        );
      if (contract.serviceType !== promoted.serviceType)
        errors.push(
          `${path}.serviceType must match the promoted ${promoted.task} contract.`
        );
      if (contract.domainId !== promoted.domainId)
        errors.push(
          `${path}.domainId must match the promoted ${promoted.task} contract.`
        );
      if (contract.name !== promoted.name)
        errors.push(
          `${path}.name must match the promoted ${promoted.task} contract.`
        );
      if (
        contract.sourcePath !==
        `${canonicalServiceSourceRoot}${promoted.sourceFile}`
      )
        errors.push(`${path}.sourcePath must match the locked Book 2 source.`);
      if (contract.implementationDepth !== 'validated_skeleton')
        errors.push(`${path}.implementationDepth must be validated_skeleton.`);
      if (
        !isPlainObject(contract.metadata) ||
        contract.metadata.implementationTask !== promoted.task
      )
        errors.push(
          `${path}.metadata.implementationTask must be ${promoted.task}.`
        );
    }

    const behaviorLock =
      behaviorLocks[contract.serviceType as keyof typeof behaviorLocks];
    ...
    const canonicalEntry =
      canonicalServiceEntries[index - existingServiceSkeletonCount];
    if (canonicalEntry !== undefined) {
      const implementationTask = canonicalEntry[4];
      if (contract.id !== `core-service-${canonicalEntry[0]}-contract`)
        errors.push(
          `${path}.id must match the locked ${implementationTask} target.`
        );
      if (contract.serviceType !== canonicalEntry[0])
        errors.push(
          `${path}.serviceType must match the locked ${implementationTask} target.`
        );
      if (contract.domainId !== canonicalEntry[1])
        errors.push(
          `${path}.domainId must match the locked ${implementationTask} target.`
        );
      if (contract.name !== canonicalEntry[2])
        errors.push(
          `${path}.name must match the locked ${implementationTask} target.`
        );
      if (
        contract.sourcePath !==
        `${canonicalServiceSourceRoot}${canonicalEntry[3]}`
      )
        errors.push(`${path}.sourcePath must match the locked Book 2 source.`);
      if (contract.implementationDepth !== 'validated_skeleton')
        errors.push(`${path}.implementationDepth must be validated_skeleton.`);
      if (!isPlainObject(contract.metadata)) {
        errors.push(
          `${path}.metadata must be present for canonical additions.`
        );
      } else {
        if (
          contract.metadata.specificationRepository !==
        ...
        if (
          contract.metadata.specificationCommit !==
        ...
        if (
          contract.metadata.specificationPath !==
          'books/book-02-core-specification/'
        )
          errors.push(
            `${path}.metadata.specificationPath must match the locked Book 2 path.`
          );
        if (contract.metadata.implementationTask !== implementationTask)
          errors.push(
            `${path}.metadata.implementationTask must be ${implementationTask}.`
          );
        if (
          implementationTask === 'CORE-TASK-023' &&
          contract.metadata.mvpRequirement !== 'stub_now'
        )
          errors.push(`${path}.metadata.mvpRequirement must be stub_now.`);
      }
    }
    ...
    for (const concept of EXCLUDED_CORE_SERVICE_CONCEPTS) {
      if (serialized.includes(concept))
        errors.push(
          `${path} must not include excluded service concept ${concept}.`
        );
    }
  });
