...
const workflowSourceRoot = `${specificationPath}core-specs/contracts/workflows/`;
const nonGoals = [
...
const canonicalWorkflowTargets = [
...
const canonicalWorkflowSkeleton = (
  workflowType: string,
  workflowName: string
): CoreWorkflowCatalogEntry => ({
  ...workflowSkeleton(
    workflowType,
    'workflow-contract',
    `Core ${workflowName} Contract Skeleton`,
    `Canonical metadata skeleton for the ${workflowName} contract boundary.`,
    `Defines the governed ${workflowName} structure and responsibility boundary without implementing transitions, execution, running instances, Task creation, Event emission, or domain mutation.`,
    [
      'canonical workflow definition reference',
      'human review gate reference'
    ]
  ),
  owns: [`${workflowName} definition, responsibility, and reference boundary.`],
  consumes: [
    'Domain and Object contract references.',
    'Permission, Policy, AI Context, and Human Review contract references.'
  ],
  produces: ['Governed workflow structure and trace-reference boundaries.'],
  nonGoals: [
    ...nonGoals,
    'Direct domain mutation, active Task creation, Event emission, external communication, or professional decisions.'
  ],
  sourcePath: `${workflowSourceRoot}${workflowType}-contract.md`,
  ...
  metadata: {
    ...
    specificationPath,
    implementationTask: 'CORE-TASK-024'
  }
});
...
export const CORE_WORKFLOW_CATALOG_SKELETONS = [
  ...
  ...canonicalWorkflowTargets.map(([workflowType, workflowName]) =>
    canonicalWorkflowSkeleton(workflowType, workflowName)
  )
