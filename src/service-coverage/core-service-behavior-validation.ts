...
export function validateCoreServiceBehaviorEvidence(
  evidence: readonly CoreServiceBehaviorEvidence[] = CORE_SERVICE_BEHAVIOR_EVIDENCE
): readonly CoreServiceBehaviorValidationIssue[] {
  const issues: CoreServiceBehaviorValidationIssue[] = [];
  if (evidence.length === 0) {
    return [issue('core.service.evidence_missing', 'Customer Service behavior evidence is missing.')];
  }
  ...
  if (entry.domainId !== 'customer' || contract?.domainId !== 'customer') {
    issues.push(
      issue('core.service.domain_mismatch', 'Customer Service evidence Domain must be customer.', 'domainId')
    );
  }
  if (entry.serviceType !== 'customer-service' || contract?.serviceType !== 'customer-service') {
    issues.push(
      issue(
  ...
  if (entry.sourcePath !== 'books/book-02-core-specification/core-specs/services/customer-service.md') {
    issues.push(
      issue('core.service.contract_mismatch', 'Customer Service evidence source path is incorrect.', 'sourcePath')
    );
  }
  if (entry.currentDepth !== 'level_2_3') {
    issues.push(issue('core.service.depth_mismatch', 'Customer Service evidence depth must be level_2_3.', 'currentDepth'));
  }
  if (!sameValues(entry.operations, CORE_CUSTOMER_IMPLEMENTED_OPERATIONS)) {
    issues.push(issue('core.service.operation_missing', 'Customer Service operations must exactly match the locked five operations.', 'operations'));
  }
  if (!sameValues(entry.provenMinimumCapabilities, CORE_CUSTOMER_MINIMUM_CAPABILITIES)) {
    issues.push(issue('core.service.capability_missing', 'Customer Service capabilities must exactly match Section 5.3 minimum capabilities.', 'provenMinimumCapabilities'));
  }
  if (!sameValues(entry.unresolvedServiceOperations, expectedUnresolved)) {
    issues.push(issue('core.service.operation_extra', 'Customer unresolved operations must exactly match the locked unresolved set.', 'unresolvedServiceOperations'));
  }
  for (const [field, paths] of [
    ...
    if (paths.length === 0 || paths.some((path) => !existsSync(path))) {
      issues.push(issue('core.service.evidence_missing', `Evidence ${field} must exist.`, field));
    }
  }
  const fixturePath = entry.fixtureFiles[0];
  if (fixturePath) {
    const fixture = JSON.parse(readFileSync(fixturePath, 'utf8')) as { fixtureType?: unknown };
    if (fixture.fixtureType !== 'core_customer_service_core_lifecycle') {
      issues.push(issue('core.service.fixture_invalid', 'Customer Service fixture validation must pass.', 'fixtureFiles[0]'));
    }
  }
  return issues;
}
