...
describe('Core Workflow Catalog Skeletons', () => {
  ...
  it('every domainId exists in CORE_DOMAIN_REGISTRY', () => { const domainIds = new Set<string>(CORE_DOMAIN_REGISTRY.map((domain) => domain.id)); for (const entry of CORE_WORKFLOW_CATALOG_SKELETONS) assert.equal(domainIds.has(entry.domainId), true); });
