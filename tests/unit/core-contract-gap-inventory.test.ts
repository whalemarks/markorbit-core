...
describe('Core Book 2 Contract Gap Inventory', () => {
  ...
  it('locks 81 unique ids and tracks all five completed batches', () => {
    const newTargets = [
      ...CORE_DOMAIN_CONTRACT_TARGETS.filter(
        (target) => target.disposition === 'add_canonical_skeleton'
      ),
    ...
    assert.equal(ids.filter((id) => currentIds.has(id)).length, 81);
    assert.equal(CORE_CONTRACT_GAP_PROGRESS.completedCanonicalTargetCount, 81);
    assert.equal(CORE_CONTRACT_GAP_PROGRESS.remainingCanonicalTargetCount, 0);
  });
