...
describe('Core Book 2 Contract Gap Inventory', () => {
  ...
  it('locks 81 unique ids and tracks the first four completed batches', () => {
    const newTargets = [
      ...CORE_DOMAIN_CONTRACT_TARGETS.filter(
        (target) => target.disposition === 'add_canonical_skeleton'
      ),
    ...
    assert.equal(ids.filter((id) => currentIds.has(id)).length, 73);
    assert.equal(CORE_CONTRACT_GAP_PROGRESS.completedCanonicalTargetCount, 73);
    assert.equal(CORE_CONTRACT_GAP_PROGRESS.remainingCanonicalTargetCount, 8);
  });
