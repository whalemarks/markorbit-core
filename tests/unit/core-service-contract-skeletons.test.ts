...
import {
  ...
  EXCLUDED_CORE_SERVICE_CONCEPTS,
...
describe('CORE_SERVICE_CONTRACT_SKELETONS', () => {
  ...
  it('validateCoreServiceContractSkeletons returns no errors', () =>
    assert.deepEqual(
  ...
  it('all skeleton ids are unique', () =>
    assert.equal(
      new Set(CORE_SERVICE_CONTRACT_SKELETONS.map((contract) => contract.id))
        .size,
      CORE_SERVICE_CONTRACT_SKELETONS.length
    ));
  it('all serviceTypes are unique', () =>
    assert.equal(
      new Set(
        CORE_SERVICE_CONTRACT_SKELETONS.map((contract) => contract.serviceType)
      ).size,
      CORE_SERVICE_CONTRACT_SKELETONS.length
    ));
  it('every domainId exists in CORE_DOMAIN_REGISTRY', () =>
    assert.ok(
      CORE_SERVICE_CONTRACT_SKELETONS.every((contract) =>
  ...
  it('each skeleton has non-empty purpose', () =>
    assert.ok(
      CORE_SERVICE_CONTRACT_SKELETONS.every(
        (contract) => contract.purpose.length > 0
      )
    ));
  it('each skeleton has owns array', () =>
    assert.ok(
      CORE_SERVICE_CONTRACT_SKELETONS.every((contract) =>
        Array.isArray(contract.owns)
      )
    ));
  ...
  it('serviceType values are kebab-case', () =>
    assert.ok(
      CORE_SERVICE_CONTRACT_SKELETONS.every((contract) =>
  ...
  it('adds exactly the 7 safe CORE-TASK-023 Service stubs', () => {
    const targets = CORE_DOMAIN_CONTRACT_TARGETS.filter(
      (target) =>
        target.implementationBatch === 'CORE-TASK-023' &&
        target.layer === 'service'
    );
    ...
    assert.ok(
      additions.every(
        (entry) =>
          entry.metadata?.implementationTask === 'CORE-TASK-023' &&
          entry.metadata.mvpRequirement === 'stub_now'
      )
    );
  });
});
