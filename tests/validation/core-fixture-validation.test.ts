...
describe('core fixture validation', () => {
  it('returns ok true for existing fixtures', async () => {
    assert.equal(
      validateCoreDomainRegistryFixture(
        await readFixture('fixtures/domains/core-domain-registry.fixture.json')
      ).ok,
      true
    );
    assert.equal(
      validateCoreObjectBaseFixture(
        await readFixture('fixtures/objects/core-object-base.fixture.json')
      ).ok,
      true
    );
    assert.equal(
      validateCoreEventBaseFixture(
        await readFixture('fixtures/events/core-event-base.fixture.json')
      ).ok,
      true
    );
    assert.equal(
      validateCoreTaskBaseFixture(
        await readFixture('fixtures/tasks/core-task-base.fixture.json')
      ).ok,
      true
    );
    assert.equal(
    ...
    assert.equal(
      validateCoreDomainContractSkeletonsFixture(
        await readFixture(
          'fixtures/contracts/core-domain-contract-skeletons.fixture.json'
        )
      ).ok,
      true
    );
    assert.equal(
      validateCoreObjectContractSkeletonsFixture(
        await readFixture(
          'fixtures/contracts/core-object-contract-skeletons.fixture.json'
        )
      ).ok,
      true
    );
    ...
    assert.equal(
      validateCoreApiContractSkeletonsFixture(
        await readFixture(
          'fixtures/contracts/core-api-contract-skeletons.fixture.json'
        )
      ).ok,
      true
    );
    assert.equal(
      validateCoreEventCatalogSkeletonsFixture(
        await readFixture(
          'fixtures/contracts/core-event-catalog-skeletons.fixture.json'
        )
      ).ok,
      true
    );
    assert.equal(
      validateCoreWorkflowCatalogSkeletonsFixture(
        await readFixture(
          'fixtures/contracts/core-workflow-catalog-skeletons.fixture.json'
        )
      ).ok,
      true
    );
    assert.equal(
      validateCorePermissionContractSkeletonsFixture(
        await readFixture(
          'fixtures/contracts/core-permission-contract-skeletons.fixture.json'
        )
      ).ok,
      true
    );
  ...
  it('service skeleton validator rejects missing serviceType', async () => {
    ...
    assert.equal(
      validateCoreServiceContractSkeletonsFixture(fixture).ok,
      false
    );
  });
  ...
  it('stub skeleton validators reject changed MVP requirement metadata', async () => {
    const objectFixture = structuredClone(
      await readFixture(
        'fixtures/contracts/core-object-contract-skeletons.fixture.json'
      )
    ) as Record<string, unknown>[];
    const serviceFixture = structuredClone(
      await readFixture(
        'fixtures/contracts/core-service-contract-skeletons.fixture.json'
      )
    ) as Record<string, unknown>[];
    const apiFixture = structuredClone(
      await readFixture(
        'fixtures/contracts/core-api-contract-skeletons.fixture.json'
      )
    ) as Record<string, unknown>[];
    (objectFixture[19].metadata as Record<string, unknown>).mvpRequirement =
      'must_build_now';
    (serviceFixture[19].metadata as Record<string, unknown>).mvpRequirement =
      'must_build_now';
    (apiFixture[26].metadata as Record<string, unknown>).mvpRequirement =
      'must_build_now';
    assert.equal(
      validateCoreObjectContractSkeletonsFixture(objectFixture).ok,
      false
    );
    assert.equal(
      validateCoreServiceContractSkeletonsFixture(serviceFixture).ok,
      false
    );
  ...
  it('workflow catalog skeleton validator rejects missing workflowType', async () => {
    ...
    assert.equal(
      validateCoreWorkflowCatalogSkeletonsFixture(fixture).ok,
      false
    );
  });
  ...
  it('permission skeleton validator rejects missing permissionType', async () => {
    ...
    assert.equal(
      validateCorePermissionContractSkeletonsFixture(fixture).ok,
      false
    );
  });
  ...
  it('policy skeleton validator rejects bypass/runtime/engine/autonomous-agent policy keywords', async () => {
    for (const keyword of [
      ...
      assert.equal(
        validateCorePolicyContractSkeletonsFixture(fixture).ok,
        false
      );
    }
  });
  ...
  it('contract coverage baseline validator rejects stale summary counts', async () => {
    ...
    assert.equal(
      validateCoreContractCoverageBaselineFixture(fixture).ok,
      false
    );
  });
